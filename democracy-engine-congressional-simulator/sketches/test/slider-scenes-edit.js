...
function democracyEngineUser() {
  ...
  function currentCongLogic() {
    ...
    if (bodyCount == 0) {
      ...
      if (count < 1 && count1 < 1 && count2 < 1) {
        ...
        offSet = dWidth / (numBodies);  // DIFFERENT FOR ORIGIN
    ...
    if (bodyCount == 2) {
      ...
      if (count < 1 && count1 < 1 && count2 < 1) {
        ...
        //Figure out how big to draw the circles and how far to space them out
        skip = floor(.97 * (sqrt(offSet * dHeight / numCon))); // DIFFERENT FOR ORIGIN
        print('Skip = ' + skip); //testing
        x = skip / 2;
        y = skip / 2;
      }
    ...
    if (bodyCount == 3) {
      strokeWeight(10);
      translate(offSet * (bodyCount), 0); // DIFFERENT FOR ORIGIN
  ...
  */
  ...
  /* MOVED PARTIAL TO ENINE, MOSTLY IN BOXES
...
/*  MOVED TO BOXES CLASS
  ...
  */
  ...
  /* MOVED TO ENGINE CLASS

  //Start the body vote box drawings
  function bodyVote() {
    fill(map(count1, 0, numCon, 0, 255));
    // reset variables if first pass thorugh function
    if (count1 < 1) {
  ...
  */
  ...
  /* MOVED PARTIAL TO ENGINE CLASS, MAINLY IN BOXES CLASS
